if ( indrogooglefonts === undefined ) {
	var indrogooglefonts = [];
}
const {
	Component,
} = wp.element;
...
class indroWebfontLoader extends Component {
	constructor(props) {
		super( props );
	...
	addFont( font ) {
		if ( ! indrogooglefonts.includes( font ) ) {
			indrogooglefonts.push( font );
		}
	};
	...
	loadFonts() {
		...
		if ( ! indrogooglefonts.includes( this.props.config.google.families[ 0 ] ) ) {
			WebFont.load( {
				...this.props.config,
...
indroWebfontLoader.propTypes = {
	config: PropTypes.object.isRequired,
	children: PropTypes.element,
	onStatus: PropTypes.func.isRequired,
};

indroWebfontLoader.defaultProps = {
	onStatus: noop,
};

export default indroWebfontLoader;
