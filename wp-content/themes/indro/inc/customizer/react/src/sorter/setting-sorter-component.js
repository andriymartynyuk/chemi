...
class SorterComponent extends Component {
	constructor() {
		...
		let defaultParams = {
			...
			imageSizes: {
				thumbnail: {
					name: __( 'Thumbnail', 'indro' ),
				},
				medium: {
					name: __( 'Medium', 'indro' ),
				},
				medium_large: {
					name: __( 'Medium Large', 'indro' ),
				},
				large: {
					name: __( 'Large', 'indro' ),
				},
	...
	render() {
		const controlLabel = (
			<Fragment>
				{/* <Tooltip text={ __( 'Reset Value', 'indro' ) }>
					<Button
		...
		return (
			<div className="indro-control-field indro-sorter-items indro-post-title-sorter">
				...
				<div className="indro-sorter-row">
					{ this.controlParams.sortable && (
						<ReactSortable animation={100} onStart={ () => this.onDragStop() } onEnd={ () => this.onDragStop() } group={ this.controlParams.group } className={ `indro-sorter-drop indro-sorter-sortable-panel indro-meta-sorter indro-sorter-drop-${ this.controlParams.group }` } handle={ '.indro-sorter-item-panel-header' } list={ theItems } setList={ ( newState ) => this.onDragEnd( newState ) } >
					...
					{ ! this.controlParams.sortable && (
						<div className={ `indro-sorter-drop indro-sorter-sortable-panel indro-sorter-no-sorting indro-sorter-drop-${ this.controlParams.group }` } >
							{ currentList.map( ( item, index ) => {
								return <ItemComponent
									key={ item }
									item={ item }
