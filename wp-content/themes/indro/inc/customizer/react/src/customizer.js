( function( $, api ) {
	...
	wp.customize.bind( 'ready', function() {
		$( 'input[name=indro-flush-local-fonts-button]' ).on( 'click', function( e ) {
			var data = {
				wp_customize: 'on',
				action: 'indro_flush_fonts_folder',
				nonce: indroCustomizerControlsData.flushFonts
			};	
			$( 'input[name=indro-flush-local-fonts-button]' ).attr('disabled', 'disabled');
	
			$.post( ajaxurl, data, function ( response ) {
				console.log( response );
				if ( response && response.success ) {
					$( 'input[name=indro-flush-local-fonts-button]' ).val( 'Successfully Flushed' );
				} else {
					$( 'input[name=indro-flush-local-fonts-button]' ).val( 'Failed, Reload Page and Try Again' );
				}
			});
		});

		wp.customize.state.create( 'indroTab' );
		wp.customize.state( 'indroTab' ).set( 'general' );
		

		// Set handler when custom responsive toggle is clicked.
		$( '#customize-theme-controls' ).on( 'click', '.indro-build-tabs-button:not(.indro-nav-tabs-button)', function( e ) {
			e.preventDefault();

			wp.customize.previewedDevice.set( $( this ).attr( 'data-device' ) );
		});
		// Set handler when custom responsive toggle is clicked.
		$( '#customize-theme-controls' ).on( 'click', '.indro-compontent-tabs-button:not(.indro-nav-tabs-button)', function( e ) {
			e.preventDefault();

			wp.customize.state( 'indroTab' ).set( $( this ).attr( 'data-tab' ) );
		});
		var setCustomTabElementsDisplay = function() {
			var tabState = wp.customize.state( 'indroTab' ).get(),
			$tabs = $( '.indro-compontent-tabs-button:not(.indro-nav-tabs-button)' );
			$tabs.removeClass( 'nav-tab-active' ).filter( '.indro-' + tabState + '-tab' ).addClass( 'nav-tab-active' );
		}
		// Refresh all responsive elements when previewedDevice is changed.
		wp.customize.state( 'indroTab' ).bind( setCustomTabElementsDisplay );

		$( '#customize-theme-controls' ).on( 'click', 'customize-section-back', function( e ) {
			wp.customize.state( 'indroTab' ).set( 'general' );
		});
		if ( indroCustomizerControlsData && indroCustomizerControlsData.contexts ) {
			/**
			 * Active callback script (JS version)
			 * ref: https://make.xwp.co/2016/07/24/dependently-contextual-customizer-controls/
			 */
			_.each( indroCustomizerControlsData.contexts, function( rules, key ) {
				var getSetting = function( settingName ) {
					...
					switch ( settingName ) {
						case '__device':
							...
							break;
						case '__current_tab':
							return wp.customize.state( 'indroTab' );
							break;
						default:
									...
									case 'load_italic':
										...
										if ( currentValue['family'] && currentValue['google'] && currentValue['variant'] ) {
											if ( 0 > currentValue['variant'].indexOf( 'italic' ) ) {
												if ( indroCustomizerControlsData.gfontvars && indroCustomizerControlsData.gfontvars[ currentValue['family'] ] && indroCustomizerControlsData.gfontvars[ currentValue['family'] ].v && indroCustomizerControlsData.gfontvars[ currentValue['family'] ].v.includes( 'italic' ) ) {
													result = true;
												}
											}
										}
				...
				if ( 0 == key.indexOf( 'indro_customizer' ) ) {
		...
		var setCustomResponsiveElementsDisplay = function() {
			var device = wp.customize.previewedDevice.get(),
			    $tabs = $( '.indro-build-tabs-button.nav-tab' );
		...
		var resizePreviewer = function() {
			var $section = $( '.control-section.indro-builder-active' );
			var $footer = $( '.control-section.indro-footer-builder-active' );
			if ( $body.hasClass( 'indro-builder-is-active' ) || $body.hasClass( 'indro-footer-builder-is-active' ) ) {
				if ( $body.hasClass( 'indro-footer-builder-is-active' ) && 0 < $footer.length && ! $footer.hasClass( 'indro-builder-hide' ) ) {
					wp.customize.previewer.container.css( 'bottom', $footer.outerHeight() + 'px' );
				} else if ( $body.hasClass( 'indro-builder-is-active' ) && 0 < $section.length && ! $section.hasClass( 'indro-builder-hide' ) ) {
		...
		var initHeaderBuilderPanel = function( panel ) {
			var section =  wp.customize.section( 'indro_customizer_header_builder' );
			if ( section ) {
				var $section = section.contentContainer,
				section_layout =  wp.customize.section( 'indro_customizer_header_layout' );
				...
				panel.expanded.bind(function( isExpanded ) {
					...
					if ( isExpanded ) {
						$body.addClass( 'indro-builder-is-active' );
						$section.addClass( 'indro-builder-active' );
						$section.css('display', 'none').height();
						$section.css('display', 'block');
					} else {
						$body.removeClass( 'indro-builder-is-active' );
						$section.removeClass( 'indro-builder-active' );
					}
				...
				$section.on( 'click', '.indro-builder-tab-toggle', function( e ) {
					e.preventDefault();
					$section.toggleClass( 'indro-builder-hide' );
					resizePreviewer();
				});
			}

		};
		wp.customize.panel( 'indro_customizer_header', initHeaderBuilderPanel );
		/**
		 * Init Header & Footer Builder
		 */
		var initFooterBuilderPanel = function( panel ) {
			var section =  wp.customize.section( 'indro_customizer_footer_builder' );
			if ( section ) {
				var $section = section.contentContainer,
				section_layout =  wp.customize.section( 'indro_customizer_footer_layout' );
				// If Header panel is expanded, add class to the body tag (for CSS styling).
				panel.expanded.bind(function( isExpanded ) {
					_.each(section.controls(), function( control ) {
						if ( 'resolved' === control.deferred.embedded.state() ) {
					...
					if ( isExpanded ) {
						$body.addClass( 'indro-footer-builder-is-active' );
						$section.addClass( 'indro-footer-builder-active' );
						$section.css('display', 'none').height();
						$section.css('display', 'block');
					} else {
						$body.removeClass( 'indro-footer-builder-is-active' );
						$section.removeClass( 'indro-footer-builder-active' );
					}
					_.each(section_layout.controls(), function( control ) {
						if ( 'resolved' === control.deferred.embedded.state() ) {
							return;
		...
		wp.customize.panel( 'indro_customizer_footer', initFooterBuilderPanel );
	});
