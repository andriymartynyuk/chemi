...
class ColorComponent extends Component {
	constructor(props) {
		...
		let baseDefault =  ( indroCustomizerControlsData.palette ? JSON.parse( indroCustomizerControlsData.palette ) : { palette: [] } );
		...
		let defaultParams = {
			...
			palettes: ( indroCustomizerControlsData.palettePresets ? indroCustomizerControlsData.palettePresets : [] ),
			colors: {
				palette1: {
					tooltip: __( '1 - Accent', 'indro' ),
					palette: false,
				},
				palette2: {
					tooltip: __( '2 - Accent - alt', 'indro' ),
					palette: false,
				},
				palette3: {
					tooltip: __( '3 - Strongest text', 'indro' ),
					palette: false,
				},
				palette4: {
					tooltip: __( '4 - Strong Text', 'indro' ),
					palette: false,
				},
				palette5: {
					tooltip: __( '5 - Medium text', 'indro' ),
					palette: false,
				},
				palette6: {
					tooltip: __( '6 - Subtle Text', 'indro' ),
					palette: false,
				},
				palette7: {
					tooltip: __( '7 - Subtle Background', 'indro' ),
					palette: false,
				},
				palette8: {
					tooltip: __( '8 - Lighter Background', 'indro' ),
					palette: false,
				},
				palette9: {
					tooltip: __( '9 - White or offwhite', 'indro' ),
					palette: false,
				},
			},
			paletteMap: {
				'palette': {
					'tooltip': __( 'Palette 1', 'indro' ),
				},
				'second-palette': {
					'tooltip': __( 'Palette 2', 'indro' ),
				},
				'third-palette': {
					'tooltip': __( 'Palette 3', 'indro' ),
				}
			}
		};
	...
	render() {
		...
		return (
				<div className="indro-control-field indro-palette-control indro-color-control">
					<div className="indro-palette-header">
					...
					<div className="indro-palette-colors">
					...
					<div className={'indro-palette-import-wrap'}>
						<Button className={ 'indro-palette-import' } onClick={ () => { this.state.isVisible ? toggleClose() : toggleVisible() } }>
							<Dashicon icon="portfolio" />
						</Button>
						{ this.state.isVisible && (
							<Popover position="bottom right" className="indro-palette-popover-copy-paste" onClose={ toggleClose }>
								<TabPanel className="indro-palette-popover-tabs"
									activeClass="active-tab"
									initialTabName={ 'import'}
									tabs={ [
										{
											name: 'import',
											title: __( 'Select a Color Set', 'indro' ),
											className: 'indro-color-presets',
										},
										{
											name: 'custom',
											title: __( 'Export/Import', 'indro' ),
											className: 'indro-export-import',
										}
									] }>
									{
										( tab ) => {
																	...
																	>
																		{ Object.keys( presetPalettes[item] ).map( ( color, subIndex ) => {
																			return (
																				...
																				} } className="indro-swatche-item-wrap">
																					<span
																						className={ 'indro-swatch-item' }
																						style={ {
																							height: '100%',
																							display: 'block',
																							width: '100%',
													...
													tabout = (
														<Fragment>
															...
															{ this.state.importError && (
																<p style={{color:'red'}}>{ __( 'Error with Import data', 'indro') }</p>
															) }
															<Button
																className={ 'indro-import-button' }
																isPrimary
					...
					{ this.props.control.params.description && (
						<span className="customize-control-description"><a href="https://indro-theme.com/docs/how-to-use-the-indro-color-palette/" target="_blank">{ this.props.control.params.description }</a></span>
					) }
				</div>
		);
	}
	...
	updateValues( value ) {
		...
		indroCustomizerControlsData.palette = JSON.stringify( value );
	}
}
