...
class ContactComponent extends Component {
	constructor() {
		...
		let defaultParams = {
			...
			'options': [
				{ value: 'phone', label: __( 'Phone', 'indro' ), content: __( '444-546-8765', 'indro' ) },
				{ value: 'hours', label: __( 'Hours', 'indro' ), content: __( 'Mon - Fri: 8AM - 5PM', 'indro' ) },
				{ value: 'email', label: __( 'Email', 'indro' ), content: __( 'example@example.com', 'indro' ) },
				{ value: 'location', label: __( 'Address', 'indro' ), content: __( '4560 example street', 'indro' ) },
			],
		};
		...
		return (
			<div className="indro-control-field indro-sorter-items">
				<div className="indro-sorter-row">
					<ReactSortable animation={100} onStart={ () => this.onDragStop() } onEnd={ () => this.onDragStop() } group={ this.controlParams.group } className={ `indro-sorter-drop indro-sorter-sortable-panel indro-sorter-drop-${ this.controlParams.group } indro-sorter-drop-social_item_group` } handle={ '.indro-sorter-item-panel-header' } list={ theItems } setList={ ( newState ) => this.onDragEnd( newState ) } >
						{ currentList.length > 0 && (
				...
				{ undefined !== availibleContactOptions[0] && undefined !== availibleContactOptions[0].value && (
					<div className="indro-contact-add-area indro-social-add-area">
						{ this.state.isVisible && (
							<Popover position="top right" className="indro-popover-color indro-popover-contact" onClose={ toggleClose }>
								<div className="indro-popover-contact-list indro-popover-social-list">
