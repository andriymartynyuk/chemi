...
import indroWebfontLoader from '../common/font-loader.js';
...
class TypographyComponent extends Component {
	constructor() {
		...
		this.state = {
			...
			activeFont: [],
			value: value,
			fontVars: ( indroCustomizerControlsData.gfontvars ? indroCustomizerControlsData.gfontvars : [] ),
			customFontVars: ( indroCustomizerControlsData.cfontvars ? indroCustomizerControlsData.cfontvars : [] ),
		};
	}
	componentDidMount() {
		let base_font;
	...
	render() {
		...
		const fontFamilyTab = (
			<Fragment>
				<div className="indro-font-family-search">
					<TextControl
						type="text"
				...
				<div className="indro-font-family-list-wrapper">
					<ButtonGroup id={ this.controlParams.id } className="indro-font-family-list" aria-label={ __( 'Font Family List' ) }>
		...
		const fontStyleTab = (
			<Fragment>
				<div className="indro-font-variant-list-wrapper">
					<ButtonGroup id={ this.controlParams.id + '-style' } className="indro-font-variant-list" aria-label={ __( 'Font Style List' ) }>
		...
		const fontSizeTab = (
			<Fragment>
				<div class="indro-range-control">
					<ResponsiveControl
						onChange={ ( currentDevice) => this.setState( { currentDevice } ) }
						controlLabel={ __( 'Font Size', 'indro' ) }
						tooltip={ false }
					>
						<RangeControl
						...
						{ this.controlParams.sizeUnits && (
							<div className="indro-select-units">
				...
				<div class="indro-range-control">
					<ResponsiveControl
						onChange={ ( currentDevice) => this.setState( { currentDevice } ) }
						controlLabel={ __( 'Line Height', 'indro' ) }
						tooltip={ false }
					>
						<RangeControl
				...
				<div class="indro-range-control">
					<ResponsiveControl
						onChange={ ( currentDevice) => this.setState( { currentDevice } ) }
						controlLabel={ __( 'Letter Spacing', 'indro' ) }
						tooltip={ false }
					>
						<RangeControl
				...
				<div class="indro-range-control indro-transform-controls">
		...
		return (
			<div className="indro-control-field indro-typography-control-wrap">
				<div className="indro-typography-control">
					{ controlLabel }
					<div className="indro-typography-controls">
						{ this.state.isVisible && 'family' === this.controlParams.options && (
							<Popover position="top right" className="indro-popover-color indro-popover-typography" onClose={ toggleClose }>
								<div className="indro-popover-typography-single-item">
									{ fontFamilyTab }
								</div>
							</Popover>
						) }
						{ this.state.isVisible && ( 'all' === this.controlParams.options || 'no-color' === this.controlParams.options ) && (
							<Popover position="top left" className="indro-popover-color indro-popover-typography" onClose={ toggleClose }>
								<TabPanel className="indro-popover-tabs indro-typography-tabs indro-background-tabs"
									...
									tabs={ [
										{
											name: 'font',
											title: __( 'Font', 'indro' ),
											className: 'indro-font-typography',
										},
										{
											name: 'style',
											title: __( 'Style', 'indro' ),
											className: 'indro-style-typography',
										},
										{
											name: 'size',
											title: __( 'Size', 'indro' ),
											className: 'indro-size-typography',
										},
									] }>
									{
										( tab ) => {
						...
						{ ( 'all' === this.controlParams.options || 'family' === this.controlParams.options || 'no-color' === this.controlParams.options ) && (
							<Tooltip text={ this.controlParams.tooltip ? this.controlParams.tooltip : __( 'Select Font', 'indro' ) }>
								<div className="typography-button-wrap">
									<Button className={ 'indro-typography-family-indicate' } onClick={ () => { this.state.isVisible ? toggleClose() : toggleVisible('font') } }>
						...
						{ ( 'all' === this.controlParams.options || 'no-color' === this.controlParams.options ) && (
							<Tooltip text={ this.controlParams.tooltip ? this.controlParams.tooltip : __( 'Select Style', 'indro' ) }>
								<div className="typography-button-wrap">
									<Button className={ 'indro-typography-style-indicate' } onClick={ () => { this.state.isVisible ? toggleClose() : toggleVisible('style') } }>
										{ this.state.value.variant ? this.state.value.variant : __( 'inherit', 'indro' ) }
									</Button>
								</div>
							</Tooltip>
						) }
						{ ( 'all' === this.controlParams.options || 'size' === this.controlParams.options || 'no-color' === this.controlParams.options ) && (
							<Tooltip text={ this.controlParams.tooltip ? this.controlParams.tooltip : __( 'Select Size', 'indro' ) }>
								<div className="typography-button-wrap">
									<Button className={ 'indro-typography-size-indicate' } onClick={ () => { this.state.isVisible ? toggleClose() : toggleVisible('size') } }>
										{ currentSize ? currentSize + this.state.value.sizeType : __( 'inherit', 'indro' ) }
									</Button>
								</div>
							</Tooltip>
						) }
						{ ( 'all' === this.controlParams.options || 'family' === this.controlParams.options || 'no-color' === this.controlParams.options ) && (
							<Tooltip text={ __( 'Show Preview Text', 'indro' ) }>
								<div className="typography-button-wrap">
									<Button className={ 'indro-typography-preview-indicate' } onClick={ () => { this.state.isPreviewVisible ? toggleClosePreview() : toggleVisiblePreview() } }>
				...
				{ this.state.value.google && (
					<indroWebfontLoader config={ configVariants } >
					</indroWebfontLoader>
				) }
				{ this.state.isPreviewVisible && (
					<div className="indro-preview-font" style={ { 
						...
						{ __( 'Design is not just what it looks like and feels like. Design is how it works.', 'indro' ) }
					</div>
				) }
			</div>
		);
