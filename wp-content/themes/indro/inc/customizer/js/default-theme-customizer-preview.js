( function( $ ) {
		...
		if ( hasSelectiveRefresh ) {
			wp.customize.selectiveRefresh.bind('partial-content-rendered', function (placement) {
				if ( placement.partial.id === 'header_desktop_items' ) {
					window.indro.initToggleDrawer();
				}
				window.indro.initTransHeaderPadding();
				if ( typeof window.indro.initStickyHeader !== "undefined" ) { 
					window.indro.initStickyHeader();
					window.indro.initScrollToTop();
				}
			} );
		}
	});
	var indroCustomizer = {
		live_css_typography : function( key, rules, newValue ) {
			var styleID = 'indro-customize-preview-css-' + key,
			    $style = $( '#' + styleID ),
				css = '',
	...
	if ( indroCustomizerPreviewData && indroCustomizerPreviewData.liveControl ) {
		_.each( indroCustomizerPreviewData.liveControl, function( liveControl, key ) {
			var eachLiveControl = {};
			...
			wp.customize( key, function( value ) {
				value.bind( function( newValue ) {
					_.each( eachLiveControl, function( rule, type ) {
						...
						indroCustomizer[ functionName ]( key, rule, newValue );
					});
				});
			});
		});
	...
	$( document ).on( 'customize-preview-menu-refreshed', function( e, params ) {
        if ( params.wpNavMenuArgs.show_toggles ) {
			window.indro.initMobileToggleSub();
        }
	});
} )( jQuery );