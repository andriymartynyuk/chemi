...
use indro_Control_Builder;
use indro_Control_Builder_Tabs;
use Customizer_Sanitize;
use WP_Customize_Control;
use LearnDash_Settings_Section;
use indro\WebFont_Loader;
...
class Theme_Customizer {
	...
	public function __construct() {
		...
		//add_action( 'wp_ajax_indro_flush_fonts_folder', array( $this, 'ajax_delete_fonts_folder' ) );
	}
	/**
	 * Add a filter that matches beaver builder custom font filter.
	 */
	public function convert_beaver_custom_fonts() {
		$beaver_fonts = apply_filters( 'indro_theme_add_custom_fonts', array() );
		if ( ! empty( $beaver_fonts ) && is_array( $beaver_fonts ) ) {
			add_filter(
				'indro_theme_custom_fonts',
	...
	public function ajax_delete_fonts_folder() {
		// Check request.
		if ( ! check_ajax_referer( 'indro-local-fonts-flush', 'nonce', false ) ) {
			wp_send_json_error( 'invalid_nonce' );
		}
		if ( ! current_user_can( 'edit_theme_options' ) ) {
			wp_send_json_error( 'invalid_permissions' );
		}
		if ( class_exists( '\indro\WebFont_Loader' ) ) {
			$font_loader = new \indro\WebFont_Loader( '' );
	...
	public function add_woocommerce_choices() {
		if ( class_exists( 'woocommerce' ) ) {
			...
			$options['cart'] = array(
				'name'    => esc_html__( 'Cart', 'indro' ),
				'section' => 'indro_customizer_cart',
			);
			...
			$options['mobile-cart'] = array(
				'name'    => esc_html__( 'Cart', 'indro' ),
				'section' => 'indro_customizer_mobile_cart',
			);
	...
	public function customizer_custom_widget_areas( $section_args, $section_id, $sidebar_id ) {
		if ( 'footer1' === $sidebar_id || 'footer2' === $sidebar_id || 'footer3' === $sidebar_id || 'footer4' === $sidebar_id || 'footer5' === $sidebar_id || 'footer6' === $sidebar_id ) {
			$section_args['panel'] = 'indro_customizer_footer';
		}
		return $section_args;
	}
	/**
	...
	public function override_defaults( $wp_customize ) {
		 ...
		 * Move Site Identity
		 */
		// $wp_customize->get_control( 'site_icon' )->section = 'indro_customizer_site_identity';
		...
		if ( isset( $wp_customize->selective_refresh ) ) {
			...
			$wp_customize->selective_refresh->add_partial(
				...
				array(
					...
					'render_callback'     => 'indro\site_branding',
				)
			);
		}
		...
		self::$live_control['indro_global_palette'] = $palette_live;
	...
	public static function get_panels() {
		...
		if ( is_null( self::$panels ) ) {
			$panels = array(
				'general' => array(
					'title'    => __( 'General', 'indro' ),
					'priority' => 18,
				),
				'header' => array(
					'title'    => __( 'Header', 'indro' ),
					'priority' => 20,
				),
				'footer' => array(
					'title'    => __( 'Footer', 'indro' ),
					'priority' => 22,
				),
				'blog_posts' => array(
					'title'    => __( 'Blog Posts', 'indro' ),
					'priority' => 23,
				),
				'custom_post' => array(
					'title'    => __( 'Custom Post Types', 'indro' ),
					'priority' => 24,
				),
			);
			if ( class_exists( 'SFWD_LMS' ) ) {
				$extra_learn['learndash'] = array(
					'title'    => __( 'LearnDash', 'indro' ),
					'priority' => 25,
				);
				$panels = array_merge(
					$panels,
	...
	public static function get_sections() {
		...
		if ( is_null( self::$sections ) ) {
			$sections = array(
				'site_identity' => array(
					'title'    => __( 'Site Identity', 'indro' ),
					'priority' => 25,
				),
				'general_layout' => array(
					'title'    => __( 'Layout', 'indro' ),
					'panel'    => 'general',
					'priority' => 7,
				),
				'sidebar' => array(
					'title'    => __( 'Sidebar', 'indro' ),
					'panel'    => 'general',
					'priority' => 8,
				),
				'sidebar_design' => array(
					'title'    => __( 'Sidebar Design', 'indro' ),
					'panel'    => 'general',
					'priority' => 8,
				),
				'colors' => array(
					'title'    => __( 'Colors', 'indro' ),
					'panel'    => 'general',
					'priority' => 9,
				),
				'general_buttons' => array(
					'title'    => __( 'Buttons', 'indro' ),
					'panel'    => 'general',
					'priority' => 10,
				),
				'general_typography' => array(
					'title'    => __( 'Typography', 'indro' ),
					'panel'    => 'general',
					'priority' => 11,
				),
				'scroll_up' => array(
					'title'    => __( 'Scroll To Top', 'indro' ),
					'panel'    => 'general',
					'priority' => 12,
				),
				...
				'general_social' => array(
					'title'    => __( 'Social Links', 'indro' ),
					'panel'    => 'general',
					'priority' => 20,
				),
				'general_comments' => array(
					'title'    => __( 'Comments', 'indro' ),
					'panel'    => 'general',
					'priority' => 20,
				),
				'general_performance' => array(
					'title'    => __( 'Performance', 'indro' ),
					'panel'    => 'general',
					'priority' => 21,
				),
				'breadcrumbs' => array(
					'title'    => __( 'Breadcrumbs', 'indro' ),
					'panel'    => 'general',
					'priority' => 20,
				),
				'general_404' => array(
					'title'    => __( '404 Page Layout', 'indro' ),
					'panel'    => 'general',
					'priority' => 20,
				),
				...
				'header_layout' => array(
					'title'    => __( 'Header Layout', 'indro' ),
					'panel'    => 'header',
					'priority' => 8,
				),
				'header_builder' => array(
					'title'    => __( 'Header Builder', 'indro' ),
					'panel'    => 'header',
					'priority' => 9,
				),
				'header_top' => array(
					'title'    => __( 'Top Row', 'indro' ),
					'panel'    => 'header',
					'priority' => 10,
				),
				'header_main' => array(
					'title'    => __( 'Main Row', 'indro' ),
					'panel'    => 'header',
					'priority' => 10,
				),
				'header_bottom' => array(
					'title'    => __( 'Bottom Row', 'indro' ),
					'panel'    => 'header',
					'priority' => 10,
				),
				'header_popup' => array(
					'title'    => __( 'Header Off Canvas', 'indro' ),
					'panel'    => 'header',
					'priority' => 10,
				),
				...
				'primary_navigation' => array(
					'title'    => __( 'Primary Navigation', 'indro' ),
					'panel'    => 'header',
					'priority' => 20,
				),
				'secondary_navigation' => array(
					'title'    => __( 'Secondary Navigation', 'indro' ),
					'panel'    => 'header',
					'priority' => 20,
				),
				'dropdown_navigation' => array(
					'title'    => __( 'Dropdown Navigation', 'indro' ),
					'panel'    => 'header',
					'priority' => 20,
				),
				...
				'mobile_navigation' => array(
					'title'    => __( 'Mobile Navigation', 'indro' ),
					'panel'    => 'header',
					'priority' => 20,
				),
				'mobile_button' => array(
					'title'    => __( 'Header Mobile Button', 'indro' ),
					'panel'    => 'header',
					'priority' => 20,
				),
				...
				'mobile_html' => array(
					'title'    => __( 'Header Mobile HTML', 'indro' ),
					'panel'    => 'header',
					'priority' => 20,
				),
				'header_html' => array(
					'title'    => __( 'HTML Editor', 'indro' ),
					'panel'    => 'header',
					'priority' => 20,
				),
				...
				'cart' => array(
					'title'    => __( 'Header Cart', 'indro' ),
					'panel'    => 'header',
					'priority' => 20,
				),
				'cart_design' => array(
					'title'    => __( 'Header Cart Design', 'indro' ),
					'panel'    => 'header',
					'priority' => 20,
				),
				'mobile_cart' => array(
					'title'    => __( 'Mobile Header Cart', 'indro' ),
					'panel'    => 'header',
					'priority' => 20,
				),
				...
				'header_search' => array(
					'title'    => __( 'Header Search', 'indro' ),
					'panel'    => 'header',
					'priority' => 20,
				),
				'transparent_header' => array(
					'title'    => __( 'Transparent Header', 'indro' ),
					'panel'    => 'header',
					'priority' => 20,
				),
				...
				'footer_layout' => array(
					'title'    => __( 'Footer Layout', 'indro' ),
					'panel'    => 'footer',
					'priority' => 8,
				),
				'footer_builder' => array(
					'title'    => __( 'Footer Builder', 'indro' ),
					'panel'    => 'footer',
					'priority' => 9,
				),
				'footer_top' => array(
					'title'    => __( 'Footer Top', 'indro' ),
					'panel'    => 'footer',
					'priority' => 10,
				),
				...
				'footer_html' => array(
					'title'    => __( 'Footer Copyright HTML', 'indro' ),
					'panel'    => 'footer',
					'priority' => 10,
				),
				'footer_social' => array(
					'title'    => __( 'Footer Social', 'indro' ),
					'panel'    => 'footer',
					'priority' => 10,
				),
				...
				'footer_navigation' => array(
					'title'    => __( 'Footer Navigation', 'indro' ),
					'panel'    => 'footer',
					'priority' => 10,
				),
				'page_layout' => array(
					'title'    => __( 'Page Layout', 'indro' ),
					//'panel'    => 'pages',
					'priority' => 22,
				),
				'search' => array(
					'title'    => __( 'Search Results', 'indro' ),
					'priority' => 24,
				),
				'search_design' => array(
					'title'    => __( 'Search Results', 'indro' ),
				...
				'post_layout' => array(
					'title'    => __( 'Single Post Layout', 'indro' ),
					'panel'    => 'blog_posts',
					'priority' => 24,
				),
			...
			if ( class_exists( 'woocommerce' ) ) {
				$extra_woo['woocommerce_product_catalog'] = array(
					'title'    => __( 'Product Catalog', 'indro' ),
					'panel'    => 'woocommerce',
					'priority' => 10,
				);
				...
				$extra_woo['product_layout'] = array(
					'title'    => __( 'Single Product Layout', 'indro' ),
					'panel'    => 'woocommerce',
					'priority' => 10,
				);
				...
				$extra_woo['account_layout'] = array(
					'title'    => __( 'My Account Layout', 'indro' ),
					'panel'    => 'woocommerce',
					'priority' => 24,
				);
				$extra_woo['woocommerce_store_notice_design'] = array(
					'title'    => __( 'Store Notice', 'indro' ),
					'panel'    => 'woocommerce',
					'priority' => 10,
				);
			...
			if ( class_exists( 'SFWD_LMS' ) ) {
				...
				$extra_learn['sfwd_design'] = array(
					'title'    => __( 'Learn Dash Design', 'indro' ),
					'panel'    => 'learndash',
					'priority' => 12,
				);
				$extra_learn['sfwd_grid_layout'] = array(
					'title'    => __( 'Course Grid Design', 'indro' ),
					'panel'    => 'learndash',
					'priority' => 12,
				);
			...
			if ( class_exists( 'LifterLMS' ) ) {
				...
				$extra_lifter['llms_membership_layout'] = array(
					'title'    => __( 'Membership Layout', 'indro' ),
					'panel'    => 'lifterlms',
					'priority' => 14,
				);
				$extra_lifter['llms_membership_archive'] = array(
					'title'    => __( 'Membership Archive', 'indro' ),
					'panel'    => 'lifterlms',
					'priority' => 15,
				);
			...
			$post_types        = indro()->get_post_types_objects();
			...
			foreach ( $post_types as $post_type_item ) {
				...
				if ( ! in_array( $post_type_name, $ignore_type, true ) ) {
					...
					$extras_post_types[ $post_type_name . '_layout' ] = array(
						'title'    => $post_type_label . ' ' . __( 'Layout', 'indro' ),
						'panel'    => 'custom_post',
						'priority' => 10,
					);
					...
					$extras_post_types[ $post_type_name . '_archive' ] = array(
						'title'    => $post_type_label . ' ' . __( 'Archive', 'indro' ),
						'panel'    => 'custom_post',
						'priority' => 10,
					);
			...
			self::$sections = apply_filters( 'indro_theme_customizer_sections', $final_sections );
		}
		// Return sections.
	...
	public static function get_capability() {
		...
		if ( is_null( self::$capability ) ) {
			self::$capability = apply_filters( 'indro_theme_customizer_capability', 'manage_options' );
		}
		// Return capability.
	...
	public function register_settings( $wp_customize ) {
		...
		foreach ( self::$settings as $setting_key => $setting ) {
			...
			if ( isset( $setting['control_type'] ) && ! empty( $setting['control_type'] ) ) {
				if ( substr( $setting['control_type'], 0, 8 ) === 'indro_' ) {
					$control_class = 'indro_Control_' . ucfirst( str_replace( 'indro_', '', str_replace( '_control', '', $setting['control_type'] ) ) );
				} else {
			...
			if ( isset( $setting['sanitize'] ) && ! empty( $setting['sanitize'] ) ) {
				if ( substr( $setting['sanitize'], 0, 8 ) === 'indro_' ) {
					$sanitize_function = array( 'indro\Customizer_Sanitize', $setting['sanitize'] );
				} else {
					$sanitize_function = $setting['sanitize'];
				}
			} else {
				$sanitize_function = array( 'indro\Customizer_Sanitize', 'indro_sanitize_option' );
				$sanitize_function = '';
			}
			if ( 'option' === indro()->get_option_type() ) {
				$setting_key = indro()->get_option_name() . '[' . $setting_key . ']';
			}
			if ( isset( $setting['settings'] ) && is_array( $setting['settings'] ) ) {
				foreach ( $setting['settings'] as $setting_key => $id ) {
					...
					$wp_customize->add_setting(
						$id,
						array(
							'type'              => indro()->get_option_type(),
							'default'           => indro()->default( $id ),
				...
				$wp_customize->add_setting(
					...
					array(
						...
						'transport'         => isset( $setting['transport'] ) ? $setting['transport'] : 'postMessage',
						'capability'        => self::get_capability(),
						'default'           => isset( $setting['default'] ) && $control_class !== 'indro_Control_Background' ? $setting['default'] : '',
						'sanitize_callback' => $sanitize_function,
					)
				);
			}
	...
	public static function maybe_add_panel( $wp_customize, $child ) {
		...
		if ( ! empty( $child['panel'] ) && isset( $panels[ $child['panel'] ] ) ) {
			...
			if ( ! in_array( $panel_key, self::$panels_added, true ) ) {
				...
				// Register panel.
				$wp_customize->add_panel( 'indro_customizer_' . $panel_key, $panel_config );
		...
		foreach ( self::$settings as $setting_key => $setting ) {
			...
			if ( ! empty( $setting['panel'] ) ) {
				if ( 'woocommerce' === $control_config['panel'] ) {
					...
					$control_config['panel'] = 'indro_customizer_' . $setting['panel'];
				}
			}
	...
	public static function active_callback( $object ) {
		if ( ! isset( $object->setting->id ) && 'indro_builder_control' !== $object->type ) {
			return true;
		}
		if ( 'indro_builder_control' === $object->type ) {
			$id = str_replace( indro()->get_option_name() . '_', '', $object->id );
		} else {
		...
		if ( ! isset( $settings[ $id ]['active_callback'][0] ) && 3 === count( $settings[ $id ]['active_callback'] ) ) {
			...
			$field_value = indro()->option( $field_id );
	...
	public static function get_builder_choices() {
		return apply_filters( 'indro_theme_customizer_control_choices', self::$choices );
	}
	/**
	...
	public function enqueue_customizer_scripts() {
		...
		if ( isset( $_GET['wpum_email_customizer'] ) || apply_filters( 'prevent_indro_customizer_scripts', false ) ) {
			return;
		}
		$palette_presets = '{"basic":[{"color":"#88D24E"},{"color":"#344E6C"},{"color":"#213C5A"},{"color":"#020D26"},{"color":"#3F485D"},{"color":"#D8E0F3"},		
{"color":"#EDF2F8"},{"color":"#F7FAFC"},{"color":"#ffffff"}],

"palette":[{"color":"#8197f3"},{"color":"#435277"},{"color":"#2d3f6d"},{"color":"#020D26"},{"color":"#3F485D"},{"color":"#D8E0F3"},{"color":"#EDF2F8"},{"color":"#F7FAFC"},{"color":"#ffffff"}],

"first-palette":[{"color":"#f1c85e"},{"color":"#5899b5"},{"color":"#216a8a"},{"color":"#020D26"},{"color":"#3F485D"},{"color":"#D8E0F3"},{"color":"#EDF2F8"},{"color":"#F7FAFC"},{"color":"#ffffff"}],

"orange":[{"color":"#6a7ed8"},{"color":"#595961"},{"color":"#34384a"},{"color":"#020D26"},{"color":"#3F485D"},{"color":"#D8E0F3"},{"color":"#EDF2F8"},{"color":"#F7FAFC"},{"color":"#ffffff"}],

"third-palette":[{"color":"#E21E51"},{"color":"#4d40ff"},{"color":"#040037"},{"color":"#020D26"},{"color":"#3F485D"},{"color":"#D8E0F3"},{"color":"#EDF2F8"},{"color":"#F7FAFC"},{"color":"#ffffff"}],

"forth-palette":[{"color":"#E21E51"},{"color":"#4d40ff"},{"color":"#f8f9fa"},{"color":"#020D26"},{"color":"#3F485D"},{"color":"#D8E0F3"},{"color":"#EDF2F8"},{"color":"#F7FAFC"},{"color":"#ffffff"}],

"fifth-palette":[{"color":"#049f82"},{"color":"#008f72"},{"color":"#222222"},{"color":"#020D26"},{"color":"#3F485D"},{"color":"#D8E0F3"},{"color":"#EDF2F8"},{"color":"#F7FAFC"},{"color":"#ffffff"}],

"sixth-palette":[{"color":"#dd6b20"},{"color":"#cf3033"},{"color":"#27241d"},{"color":"#020D26"},{"color":"#3F485D"},{"color":"#D8E0F3"},{"color":"#EDF2F8"},{"color":"#F7FAFC"},{"color":"#ffffff"}]}';
		$path     = get_template_directory_uri() . '/inc/customizer/';
		$dir_path = get_template_directory() . '/inc/customizer/';
		wp_enqueue_style( 'indro-customizer-styles', $path . 'css/default-theme-customizer.css', false, VERSION );
		if ( is_rtl() ) {
			wp_enqueue_style( 'indro-customizer-rtl', $path . 'css/rtl-default-theme-customizer.css', false, VERSION );
		}
		...
		wp_enqueue_script( 'indro-customizer-controls', get_template_directory_uri() . '/assets/js/customizer.js', $editor_dependencies, VERSION, true );
		wp_enqueue_style( 'indro-customizer-controls', $path . 'react/build/controls.css', array( 'wp-components' ), VERSION );
		wp_localize_script(
			'indro-customizer-controls',
			'indroCustomizerControlsData',
			array(
				'contexts'       => self::get_control_contexts(),
				'choices'        => self::get_builder_choices(),
				'palette'        => indro()->get_palette_for_customizer(),
				'gfontvars'      => $google_font_variants,
				'cfontvars'      => apply_filters( 'indro_theme_custom_fonts', array() ),
				'palettePresets' => apply_filters( 'indro_theme_palette_presets', $palette_presets ),
				'flushFonts'     => wp_create_nonce( 'indro-local-fonts-flush' ),
			)
		);
		$css = ':root {
			--global-palette1: ' . indro()->palette_option( 'palette1' ) . ';
			--global-palette2: ' . indro()->palette_option( 'palette2' ) . ';
			--global-palette3: ' . indro()->palette_option( 'palette3' ) . ';
			--global-palette4: ' . indro()->palette_option( 'palette4' ) . ';
			--global-palette5: ' . indro()->palette_option( 'palette5' ) . ';
			--global-palette6: ' . indro()->palette_option( 'palette6' ) . ';
			--global-palette7: ' . indro()->palette_option( 'palette7' ) . ';
			--global-palette8: ' . indro()->palette_option( 'palette8' ) . ';
			--global-palette9: ' . indro()->palette_option( 'palette9' ) . ';
			--global-palette-highlight: ' . $this->render_color( indro()->sub_option( 'link_color', 'highlight' ) ) . ';
			--global-palette-highlight-alt: ' . $this->render_color( indro()->sub_option( 'link_color', 'highlight-alt' ) ) . ';
			--global-palette-highlight-alt2: ' . $this->render_color( indro()->sub_option( 'link_color', 'highlight-alt2' ) ) . ';
			--global-palette-btn: ' . $this->render_color( indro()->sub_option( 'buttons_color', 'color' ) ) . ';
			--global-palette-btn-hover: ' . $this->render_color( indro()->sub_option( 'buttons_color', 'hover' ) ) . ';
			--global-palette-btn-bg: ' . $this->render_color( indro()->sub_option( 'buttons_background', 'color' ) ) . ';
			--global-palette-btn-bg-hover: ' . $this->render_color( indro()->sub_option( 'buttons_background', 'hover' ) ) . ';
			--global-base-font: ' . indro()->sub_option( 'base_font', 'family' ) . ';
			--global-heading-font: ' . ( 'inherit' !== indro()->sub_option( 'heading_font', 'family' ) ? indro()->sub_option( 'heading_font', 'family' ) : 'var(--global-base-font)' ) . ';
		}';
		wp_add_inline_style( 'indro-customizer-controls', wp_strip_all_tags( $css ) );
	}
	/**
	...
	public function action_enqueue_customize_preview_scripts() {
		$path = get_template_directory_uri() . '/inc/customizer/';
		wp_enqueue_style( 'indro-customizer-preview', $path . 'css/default-theme-customizer-preview.css', false, VERSION );
		wp_enqueue_script( 'indro-webfont-js', 'https://ajax.googleapis.com/ajax/libs/webfont/1.6.26/webfont.js', array( 'jquery' ), '1.6.26', true );
		wp_enqueue_script( 'indro-customizer-preview', $path . 'js/default-theme-customizer-preview.min.js', array( 'customize-preview' ), VERSION, true );
		wp_localize_script(
			'indro-customizer-preview',
			'indroCustomizerPreviewData',
			array(
	...
	public static function get_control_contexts() {
		// Return contexts.
		return apply_filters( 'indro_theme_customizer_control_contexts', self::$contexts );
	}
	...
	public static function get_live_control_data() {
		// Return contexts.
		return apply_filters( 'indro_theme_customizer_live_control_data', self::$live_control );
	}
