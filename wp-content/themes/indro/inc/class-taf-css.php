 ...
 *
 * @author  Carlos Rios - Edited by Ben Ritner for use in indro Theme
 * @package  indro
 * @version  1.1
 */
...
class indro_CSS {
	...
	public function render_font_family( $font, $area = 'headers' ) {
		...
		return apply_filters( 'indro_theme_font_family_string', $font_string );
	}
	...
	public function render_font( $font, $css, $inherit = null ) {
		...
		if ( ! empty( $family ) ) {
			...
			$css->add_property( 'font-family', apply_filters( 'indro_theme_font_family_string', $family ) );
			if ( isset( $font['google'] ) && true === $font['google'] ) {
				if ( ! empty( $inherit ) && 'body' === $inherit ) {
	...
	public function maybe_add_google_font( $font, $full = null ) {
		...
		} elseif ( ! empty( $full ) && 'body' === $full && 'inherit' !== indro()->sub_option( 'heading_font', 'family' ) ) {
			$new_variant = array( $font['variant'], '700' );
		}
		if ( ! empty( $full ) && 'body' === $full ) {
			if ( indro()->option( 'load_base_italic' ) ) {
