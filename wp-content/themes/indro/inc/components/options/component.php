<?php
/**
 * indro\Options\Component class
 *
 * @package indro
 */

namespace indro\Options;
 ...
 * * `indro()->option()`
 * * `indro()->default()`
 * * `indro()->get_option_type()`
 * * `indro()->get_option_name()`
 * * `indro()->sub_option()`
 * * `indro()->sidebar_options()`
 * * `indro()->palette_option()`
 * * `indro()->get_palette()`
 */
class Component implements Component_Interface, Templating_Component_Interface {
	...
	public function get_option_type() {
		...
		if ( is_null( self::$opt_type ) ) {
			self::$opt_type = apply_filters( 'indro_theme_option_type', 'theme_mod' );
		}
		// Return option_type.
	...
	public function get_option_name() {
		...
		if ( is_null( self::$opt_name ) ) {
			self::$opt_name = apply_filters( 'indro_theme_option_name', 'indro_settings' );
		}
		// Return option_name.
		...
		if ( is_null( self::$default_options ) ) {
			self::$default_options = apply_filters(
				...
				array(
					...
					// Header HTML.
					'header_html_content'    => __( 'Insert HTML here', 'indro' ),
					...
					// Header Button.
					'header_button_label'      => __( 'Button', 'indro' ),
					'header_button_link'      => '',
					...
					'mobile_button_label'      => __( 'Button', 'indro' ),
					...
					// Mobile Header HTML.
					'mobile_html_content'    => __( 'Insert HTML here', 'indro' ),
					...
					'product_content_element_price' => array(
						'enabled' => true,
						'show_shipping' => false,
						'shipping_statement' => __( '& Free Shipping', 'indro' ),
					),
					...
					'product_content_element_extras' => array(
						'enabled'   => false,
						'title'     => __( 'Free shipping on orders over $50!', 'indro' ),
						'feature_1' => __( 'Satisfaction Guaranteed', 'indro' ),
						'feature_2' => __( 'No Hassle Refunds', 'indro' ),
						'feature_3' => __( 'Secure Payments', 'indro' ),
						'feature_4' => '',
						'feature_5' => '',
					...
					'product_content_element_payments' => array(
						'enabled' => false,
						'title'     => __( 'GUARANTEED SAFE CHECKOUT', 'indro' ),
						'visa' => true,
						'mastercard' => true,
						'amex' => true,
						'discover' => true,
	...
	public function get_palette_for_customizer() {
		$palette = get_option( 'indro_global_palette' );
	...
	public function palette_option( $subkey ) {
		...
		$active = apply_filters( 'indro_active_palette', ( self::$palette && is_array( self::$palette ) && isset( self::$palette['active'] ) && ! empty( self::$palette['active'] ) ? self::$palette['active'] : 'palette' ) );
		$value = '';
		...
		return apply_filters( 'indro_palette_option', $value, $subkey );
	}
